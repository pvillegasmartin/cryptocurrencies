...
def create_data(file='C:/Users/Pablo/Desktop/PMG/00Versions/get_data/BTCUSDT-4h.csv', period='4H', output=10,
    ...
    # Order final
    col_study = ['Output', 'Close', 'High', 'Low', 'Open', 'Volume', 'NumberOfTrades']
    df = df[col_study]
...
if __name__ == '__main__':
    ...
    profit = 10
    if not training:
        filename = f'Decision Tree_profit+10.sav'
        eval_year = 2022
    # ----------------------------
    ...
    if training:
        ...
        if filename.split()[0] == "Decision":
            graph = Source(export_graphviz(loaded_model, out_file=None, feature_names=x_new.columns[:-3], class_names=True))
            graph.format = 'png'
